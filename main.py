...
df = pd.read_csv("real estate.csv") # Enter dataset name here

print(df.head())
# What is the shape of the data?

shape_data = (len(df.axes[0]), len(df.axes[1]))# 1.1 of Gradescope tests
print(shape_data)
...
dataframe.drop(dataframe[dataframe['Y house price of unit area'] > 80].index, inplace=True)# YOUR CODE HERE
dataframe = dataframe[dataframe['X3 distance to the nearest MRT station'] <= 2800]
dataframe.drop(dataframe[dataframe['X6 longitude'] < 121.50].index, inplace=True)
...
dataframe['Y house price of unit area'] = dataframe['Y house price of unit area'].apply(lambda x: x*91)# YOUR CODE HERE
...
# Convert the scaled NumPy array back to a DataFrame (if needed)
normalized = pd.DataFrame(normalized, columns=dataframe.columns)

# Note make sure you pay heed to the data type of normalized

 # YOUR CODE HERE to convert normalized back to a dataframe
mean_norm_Y = normalized['Y house price of unit area'].mean()# Put the mean value of the 'Y house price of unit area' column after Norm, here - 1.9 of Gradescope tests
print(mean_norm_Y)
...
mean_val_conv_7 =  round(grouped_age_price_df.iloc[7], 2)# Put the value here -- 1.17 of Gradescope tests
